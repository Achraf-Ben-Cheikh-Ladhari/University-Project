...
<head>
    ...
    <title>Admin | Supprimer Etudiant</title>
</head>
<body>
    <div class="form">
        <div class="right-b"></div>
        <div class="login">
            <h2 class="Connect">Admin | Supprimer Etudiant</h2>
...
if (isset($_POST["envoyer"])){
    $mat=$_POST['mat'];
    $affiche="select name from students where mat_student='$mat'";
    $aff=mysqli_query($conn,$affiche);
    $requet="delete from users where password='$mat'";
    $requet2="delete from students where mat_student='$mat'";
    $ok=mysqli_query($conn,$requet); 
    $ok2=mysqli_query($conn,$requet2); 
    if (mysqli_num_rows($aff) > 0) {
        while($row = mysqli_fetch_array($aff)){
            if ($ok && $ok2 ){
                $_SESSION['status']="L'étudiant ".$row['name']." a été supprimer !";
                header("location:supet.php");
            }
        }
    }else{
